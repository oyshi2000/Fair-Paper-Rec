...
def prepare_data(dataset, protected_group_column, device, test_size=0.2, random_state=42):
    ...
    input_features = dataset.drop(columns=protected_group_column + ['rating', 'Title', 'Authors'])
...
def select_top_papers(model, input_features, device, df, desired_acceptance_count=323):
    """
